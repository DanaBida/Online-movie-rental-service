...
/*
 * contains functions that returns server of one type from the 2, 
 * and an un-defult method - serve, that listening and handling new clients.
 */
public interface Server<T> extends Closeable {
    ...
    public static <T> Server<T> threadPerClient( int port, Supplier<BidiMessagingProtocol<T> > protocolFactory,
		...
		//returns new BaseServer from the threadPerClient type
     ...
     */
    public static <T> Server<T> reactor( int numOfThreads, int port, Supplier<BidiMessagingProtocol<T>> protocolFactory,
            Supplier<MessageEncoderDecoder<T>> encoderDecoderFactory) {
		
    	//returns new Reactor server
        return new Reactor<T>(numOfThreads, port, protocolFactory, encoderDecoderFactory);
    }
